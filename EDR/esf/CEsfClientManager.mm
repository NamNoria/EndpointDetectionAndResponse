...
CEsfClientManager::CEsfClientManager()
{
    try
    {
        ...
        auto notifyHandler = ^(es_client_t *client, const es_message_t *message) {
            ...
            if (!node)
            {
                LOG_ERROR("Failed to allocate ESFEvent for notify");
            ...
            // 注意：PushNotifyEvent失败时会自行清理node和message
        };
        ...
        auto authHandler = ^(es_client_t *client, const es_message_t *message) {
            ...
            if (!node)
            {
                LOG_ERROR("Failed to allocate ESFEvent for auth");
                es_release_message(message);
                return;
            }
            ...
            // 注意：PushAuthEvent失败时会自行清理node和message
        };
...
CEsfClientManager::~CEsfClientManager()
{
    ...
    if (m_pNotifyClient)
    {
        LOG_INFO("Releasing notify client");
        es_delete_client(m_pNotifyClient);
        m_pNotifyClient = nullptr;
    }

    // 释放 Auth 客户端
    if (m_pAuthClient)
    {
        LOG_INFO("Releasing auth client");
        es_delete_client(m_pAuthClient);
        m_pAuthClient = nullptr;
    }

    LOG_INFO("CEsfClientManager cleanup complete");
}
        ...
        for (auto eventType : notifyEvents)
        {
            ...
            switch (eventType)
            {
                ...
                case ES_EVENT_TYPE_NOTIFY_EXEC:
                    ...
                    break;
                default:
                    typeName = "NOTIFY_" + std::to_string(static_cast<int>(eventType));
                    break;
            }
        ...
        for (auto eventType : authEvents)
        {
            ...
            switch (eventType)
            {
                ...
                case ES_EVENT_TYPE_AUTH_MOUNT:
                    ...
                    break;
                default:
                    typeName = "AUTH_" + std::to_string(static_cast<int>(eventType));
                    break;
            }

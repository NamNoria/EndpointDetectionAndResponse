...
void CEsfDispatcher::PushNotifyEvent(ESFEvent *event)
{
    // 参数检查，失败时清理资源
    if (!event)
    {
    ...
    if (!event->message)
    {
        LOG_ERROR("PushNotifyEvent: event->message is null, cleaning up");
        free(event);
        return;
    }

    /*if( ES_EVENT_TYPE_NOTIFY_CLOSE == event->message->event_type )
    {
        const audit_token_t token = event->message->process->audit_token;
    ...
    m_queNotifyEvent.push(event);
    pthread_mutex_unlock(&m_queNotifyMutex);
    pthread_cond_signal(&m_queNotifyCond);
}

void CEsfDispatcher::PushAuthEvent(ESFEvent *event)
{
    // 参数检查，失败时清理资源
    if (!event)
    {
    ...
    if (!event->message)
    {
        LOG_ERROR("PushAuthEvent: event->message is null, cleaning up");
        free(event);
        return;
    }
    ...
    {
        // 丢弃最老的事件，但必须响应AUTH事件防止系统hang
        LOG_WARN("Auth event queue full, dropping oldest event");
        ESFEvent *oldEvent = m_queAuthEvent.front();
        m_queAuthEvent.pop();

        // 关键：必须响应AUTH事件，否则系统会hang
        es_respond_auth_result(oldEvent->client, oldEvent->message,
...
//            LOG_INFO("ES_EVENT_TYPE_AUTH_EXEC");
            result = es_respond_auth_result(client, message, authResult, false);
            break;
            
        case ES_EVENT_TYPE_AUTH_CREATE:
            // create 事件使用 auth_result
//            LOG_INFO("ES_EVENT_TYPE_AUTH_CREATE");
            result = es_respond_auth_result(client, message, authResult, false);
            break;
            
        case ES_EVENT_TYPE_AUTH_RENAME:
            // rename 事件使用 auth_result
//            LOG_INFO("ES_EVENT_TYPE_AUTH_RENAME");
            result = es_respond_auth_result(client, message, authResult, false);
            break;
            
        case ES_EVENT_TYPE_AUTH_MOUNT:
//            LOG_INFO("ES_EVENT_TYPE_AUTH_MOUNT");
            result = es_respond_auth_result(client, message, authResult, false);
            break;
...
bool CEsfDispatcher::dispatchAuthEvent(es_event_type_t eventType, const es_message_t *message)
{
    ...
    if ( it != m_eventSubscriptions.end() )
    {
        ...
        for ( auto *observer: observers )
        {
            if ( !observer )
            {
                LOG_WARN("Null observer in auth event dispatch, event_type={}, skipping", static_cast<int>(eventType));
                continue;  // 跳过空指针，继续处理后续 observers
            }
            try
            {
...
{
//    LOG_INFO("dispatchNotify Eventevent_type={}", static_cast<int>(eventType));
    if ( !message )
    {
    ...
    if ( it != m_eventSubscriptions.end() )
    {
        ...
        for ( auto *observer: observers )
        {
            if ( !observer )
            {
                LOG_WARN("Null observer in notify event dispatch, event_type={}, skipping", static_cast<int>(eventType));
                continue;  // 跳过空指针，继续处理后续 observers
            }
            try
            {
...
//    CEsfDispatcher *self = static_cast<CEsfDispatcher *>(arg);
    while (true)
    {
        ...
        {
            LOG_INFO("Auth queue size high: {}, worker_tid={}", queueSize, (void*)tid);
        }
